...
<section class="Hui-article-box">
  <nav class="breadcrumb"><i class="Hui-iconfont">&#xe681;</i> 内容管理 <span class="c-gray en">&gt;</span> 标签 <span class="c-gray en">&gt;</span> 管理标签 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
    ...
    <article class="cl pd-20">
      <div class="cl pd-5 bg-1 bk-gray">
        <span class="l">
          ...
          <a class="btn btn-primary radius" href="javascript:;" onclick="admin_role_add('添加标签','./admin/label/add','800')">
            <i class="Hui-iconfont">&#xe600;</i> 添加标签
          </a>
        </span>
        <span class="r">共有数据：
          <strong id="labelRows"> </strong> 条
        </span>
      </div>
      <div class="mt-20">
        <div id="DataTables_Table_0_wrapper" class="dataTables_wrapper no-footer">
          <div id="DataTables_Table_0_filter" class="dataTables_filter">
            ...
            <label>从当前数据中检索：
              <input type="search" id="name" class="" placeholder="标签" aria-controls="DataTables_Table_0">
...
<script type="text/javascript">
  ...
  function selectLabelList(pageNo){
    var name=$("#name").val();
    var pageSize=10;
    //发送请求
    $.ajax({
      url:'admin/label/list.do',
      data:{
        name:name,
        pageNo:pageNo,
        pageSize:pageSize
      ...
      success:function(data){
        //显示总条数
        $("#labelRows").text(data.labelRows);
        //显示市场活动的列表
        var htmlStr="";
        $.each(data.labelList,function (index,obj){
          htmlStr+="<tr class=\"text-c odd\" role=\"row\">";
          htmlStr+="<td><input type=\"checkbox\" id=\""+obj.lid+"\" name=\""+obj.lid+"\"></td>";
          htmlStr+="<td>"+obj.lid+"</td>";
          htmlStr+="<td>"+obj.name+"</td>";
          htmlStr+="<td class=\"td-manage\"> <a title=\"编辑\" href=\"javascript:;\" onclick=\"admin_role_edit('编辑','./admin/label/edit?lid="+obj.lid+"','4','','510')\" class=\"ml-5\" style=\"text-decoration:none\"><i class=\"Hui-iconfont\"></i></a></td>";
          htmlStr+="</tr>";
        });
        $("#tBody").html(htmlStr);
        ...
        //计算总页数
        var totalPages=1;
        if(data.labelRows%pageSize==0){
          totalPages=data.labelRows/pageSize;
        }else
          totalPages=parseInt(data.labelRows/pageSize)+1;

        //嗲用bs_pagination工具函数
        $("#labelList").bs_pagination({
          currentPage:pageNo,//当前页号,相当于pageNo

          rowsPerPage:10,//每页显示条数,相当于pageSize
          totalRows:data.labelRows,//总条数
          ...
          onChangePage: function(event,pageObj) { // returns page_num and rows_per_page after a link has clicked
            ...
            selectLabelList(pageObj.currentPage);
          }
        });
      }
    })}
